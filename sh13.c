   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				if ((mx<200) && (my<50) && (connectEnabled==1))
				{
					...
					// On envoie le message de connexion au serveur principal
    				sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);    //Code ajouté
					connectEnabled=0;
				}
		...
		switch (gbuffer[0])
		{
			...
			case 'I':
				// RAJOUTER DU CODE ICI
				// sscanf va lire l'entier après le 'I' et le mettre dans gId.
				sscanf(gbuffer, "I %d", &gId);
				printf("Mon ID est : %d\n", gId); // code ajouté : pour vérifier dans le terminal
				break;
			...
			case 'M':
			{
				// RAJOUTER DU CODE ICI
				int idCourant;
				sscanf(gbuffer, "M %d", &idCourant);
				if (idCourant == gId) goEnabled = 1;
				else goEnabled = 0;
				break;
			}
